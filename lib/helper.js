        ...
        if (osType === 'Windows_NT') {
            for (let dev in netInfo) {
                ...
                //  if (dev === '本地连接' || dev === '以太网' || dev.indexOf('本地连接*')==0 ) {
                 if (dev === 'WLAN' || dev === 'WLAN 4' || dev ===  'NIC1'||dev === '本地连接' || dev === '以太网') {
...
function writekv_version1(Config, apis) {
    let name = Config.name;
    let keys = Object.keys(apis);
    let aa = consul.kv.set(name + 'v1/apis' , JSON.stringify(apis));
}


module.exports={
    getLocalIP:getLocalIP,
    writekv_version1:writekv_version1,
